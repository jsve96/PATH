...
class KoVAE(nn.Module):
    ...
    def sample_prior(self, n_sample, seq_len, random_sampling=True):
        ...
        h_t = torch.zeros(batch_size, self.hidden_dim).to(self.args.device)
    ...
    def zeros_init(self, batch_size, seq_len):
        ...
        z_out = torch.zeros(batch_size, seq_len, self.z_dim).to(dev)
        z_means = torch.zeros(batch_size, seq_len, self.z_dim).to(dev)
        z_logvars = torch.zeros(batch_size, seq_len, self.z_dim).to(dev)
