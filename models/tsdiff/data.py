...
class DataModule(LightningDataModule):
    ...
    def _load_dataset(self):
        filepath = f'data/{self.name}/train.npy'
        ##(M,N,d) or (M,T,d)
        data = np.load(filepath)
        M,N,d = data.shape
        ## tsdiff needs x (M,N,d) and t (M,N,1) 
        t = torch.arange(N).float().view(1, N, 1).repeat(M, 1, 1)
        dataset = TensorDataset(torch.Tensor(data), torch.Tensor(t))
        return dataset
